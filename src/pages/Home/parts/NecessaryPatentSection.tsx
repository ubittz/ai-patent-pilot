...
function NecessaryPatentSection({ onClickButton }: { onClickButton: (page: asType<typeof PAGES>) => void }) {
  const SLIDER_ITEM = [
    {
      id: 1,
      title: '내게 필요한 등록 특허 찾기',
      page: PAGES.FIND_PATENT,
      description: (
  ...
  ].map(({ id, title, description, image, page }) => (
    <SliderItem
      key={id}
      title={title}
      image={image}
      hiddenButton={false}
      buttonColor={COLORS.SUB_GREEN}
      imageHeight={194}
